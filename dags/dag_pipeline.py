...
def init(ti, job_id:str, **context):
    """
    Initialize the job_id and the start_date.
...
def get_data(ti, mode:str="training"):
    ...
    helpers.log_activity(job_id, job_type=mode, stage="etl", status="started", message="", job_date=start_date)
    try:
        start_date = datetime.datetime.strptime(start_date, "%Y-%m-%d").date()
        etl.collect_data(job_id=job_id)
        helpers.log_activity(job_id, job_type=mode, stage="etl", status="pass", message="", job_date=start_date)
    except Exception as e:
        ti.xcom_push(key='exception', value=str(e))
        message = str(traceback.format_exc())
        helpers.log_activity(job_id, job_type=mode, stage="etl", status="failed", message=message, job_date=start_date)
...
def preprocess_data(ti, mode:str=None):
    ...
    job_id = ti.xcom_pull(key='job_id', task_ids=f'_{DAG_ID}__init_')
    last_deployed_job_id = ti.xcom_pull(key='last_deployed_job_id', task_ids=f'_{DAG_ID}__init_')
    date = ti.xcom_pull(key='start_date', task_ids=f'_{DAG_ID}__init_')
    helpers.create_table_ml_job()
